...
nvat_rc_t attest(char* json_file_path);
void teardown(context_t ctx);
char *get_nonce_from_evidence(char *file_path);

#define DEFAULT_EVIDENCE_FILE "../common-test-data/serialized_test_evidence/hopper_evidence.json";

int main(int argc, char* argv[]) {
    char *evidence_file = DEFAULT_EVIDENCE_FILE;
    if(argc == 2) {
        evidence_file = argv[1];
    }

    nvat_rc_t rc = attest(evidence_file);
    if (rc != NVAT_RC_OK) {
        fprintf(stderr, "json_gpu_evidence failed: %s (nvat code: %03d)\n", nvat_rc_to_string(rc), rc);
        return 1;
    }
    return 0;
}
...
nvat_rc_t attest(char* json_file_path) {
    ...
    err = nvat_gpu_evidence_source_from_json_file(&ctx.evidence_source, json_file_path);
    ...
    const char *nonce = get_nonce_from_evidence(json_file_path);
    
    err = nvat_nonce_from_hex(&ctx.nonce, nonce);
    if (err != NVAT_RC_OK) {
        teardown(ctx);
        return err;
    }
    ...
    // nvat_gpu_evidence_collect gathers attestation evidence from the local file
...
char *get_nonce_from_evidence(char *file_path) {
    // TODO Read the JSON file and extract this value.
    return "e97b23a1718095a0e9e35edca810768c70a6a5a389b705e753b197912bc11576";
}
