...
namespace nvattestation {
    ...
    Error ClaimsCollection::get_detached_eat(
        ...
        std::string issuer_to_use;
        if (issuer.empty()) {
            issuer_to_use = "NVAT-LOCAL-VERIFIER";
        } else {
        ...
        std::unordered_map<std::string, int> device_index_for_submod_label;
        ...
        for (const auto& claim : m_claims) {
            ...
            if (!kid.empty()) {
                token.set_header_claim("kid", jwt::basic_claim<jwt::traits::nlohmann_json>(kid));
            }
            std::string submod_jwt_token;
            if (private_key_pem.empty()) {
                submod_jwt_token = token.sign(jwt::algorithm::none{});
            } else {
                submod_jwt_token = token.sign(jwt::algorithm::es384("", private_key_pem, "", ""));
            }
        ...
        if (!kid.empty()) {
            overall_jwt_token.set_header_claim("kid", jwt::basic_claim<jwt::traits::nlohmann_json>(kid));
        }
        if (private_key_pem.empty()) {
            detached_eat.m_overall_jwt_token = overall_jwt_token.sign(jwt::algorithm::none{});
        } else {
            detached_eat.m_overall_jwt_token = overall_jwt_token.sign(jwt::algorithm::es384("", private_key_pem, "", ""));
        }
    ...
    void from_json(const nlohmann::json& json, SerializableEATSubmodClaims& submod_claims) {
        ...
        if (json.contains("x-nvidia-gpu-attestation-report-cert-chain")) {
            ...
            std::shared_ptr<SerializableGpuClaimsV3> gpu_claims = std::make_shared<SerializableGpuClaimsV3>();
            ...
            std::shared_ptr<SerializableSwitchClaimsV3> switch_claims = std::make_shared<SerializableSwitchClaimsV3>();
