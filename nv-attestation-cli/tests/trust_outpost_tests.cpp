...
2. Claims should indicate that OCSP nonce does not match (x-nvidia-cert-ocsp-nonce-matches = false)
    ...
    ASSERT_EQ(exit_code, NVAT_RC_OVERALL_RESULT_FALSE) << "Expected exit code " << NVAT_RC_OVERALL_RESULT_FALSE << " but got " << exit_code << " (cached OCSP response has mismatched nonce). Command: " << redact_cmd(cmd) << "\nOutput:\n" << output;
